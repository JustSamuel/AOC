...
const parseInput = (rawInput: string): {steps: string[], grid: string[][], robot: Coord, boxes: Map<string, Box>} => {
...
function findPushableBoxes(grid: string[][], boxes: Map<string, Box>, current: Coord, step: string, chain: Set<string>): Set<string> {
...
function move(grid: string[][], robot: Coord, boxes: Map<string, Coord>, step: string) {
  ...
  if (target.x < 0 || target.y < 0 || target.x >= grid[0].length || target.y >= grid.length) return;
  let targetCell = grid[target.y][target.x];
  if (targetCell === '#') return;

  if (boxes.has(getKey(target))) {
    try {
      const keys = findPushableBoxes(grid, boxes, {x: robot.x, y: robot.y}, step, new Set ());
...
const part22 = (rawInput: string): number => {
  ...
  // @ts-ignore
  xmap = xmap.replaceAll(".","..").replaceAll("@","@.").replaceAll("#","##").replaceAll("O","[]");
